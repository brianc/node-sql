...
Postgres.prototype._getParameterValue = function(value) {
  ...
  if (null === value) {
    ...
    if (_.isArray(value)) {
      ...
      value = '(' + value.join(', ') + ')';
    } else if (_.isFunction(value.toUTCString)) {
      // Date object's default toString format does not get parsed well
      // Handle date like objects using toUTCString
      value = this._getParameterValue(value.toUTCString());
    } else {
...
Postgres.prototype.visitOrderByColumn = function(column) {
  var text = this.visit(column.column);
  if (column.direction) {
    text += ' ' + this.visit(column.direction);
  }
  return [text];
};
