...
Postgres.prototype.getQuery = function(queryNode) {
  this.visit(queryNode);
  //automatic 'from'
  if(!this._visitedFrom) {
    var from = new From().add(queryNode.table.toNode());
    var fromText = this.visitFrom(from);
    var out = [];
    out = out.concat(this.output.slice(0, this._selectEndIndex));
    out = out.concat(fromText)
    out = out.concat(this.output.slice(this._selectEndIndex));
    this.output = out;
  }
