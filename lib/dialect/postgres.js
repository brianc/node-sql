...
Postgres.prototype.visitCreateIndex = function(node) {
  ...
  var indexNode = Table.define({ name: node.indexName(), columns: [] }).toNode();
  var result    = [ 'CREATE' ];
  ...
  result = result.concat([ 'INDEX', this.visit(indexNode) ]);
  ...
  result = result.concat([
    ...
    "(" + node.options.columns.reduce(function(result, col) {
      return result.concat(this.visit(col.toNode()));
    }.bind(this), []) + ")"
  ]);
...
Postgres.prototype.visitDropIndex = function(node) {
  ...
  result.push(this.visit(Table.define({ name: node.options.indexName, columns: [] }).toNode()));
  result.push("ON");
