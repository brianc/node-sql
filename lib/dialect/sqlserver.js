 ...
 * @returns {String[]}
 */
SqlServer.prototype.visitQueryHelper=function(actions,targets,filters){
	/**
	 *
	 * @param {Node[]} list
	 * @param {String} type
	...
	function _handleLimitAndOffset(){
		...
		if (offsetInfo) throw new Error("MS SQL Server does not allow OFFSET without ORDER BY");
		if (limitInfo) _processLimit(limitInfo);
	}
...
SqlServer.prototype.visitSelect = function(select) {
	if (!select.msSQLLimitNode) return SqlServer.super_.prototype.visitSelect.call(this, select);
	var result=[
		'SELECT',
